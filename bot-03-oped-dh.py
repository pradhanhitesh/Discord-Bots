...
url = "https://www.deccanherald.com/opinion-editorial"
...
if response.status_code == 200:
    ...
    ece_links = [a['href'] for a in soup.find_all('a', href=True) if a['href'].startswith('/opinion/editorial')]
    ece_links = [f"https://www.deccanherald.com{x}" for x in ece_links]
...
for link in ece_links:
    ...
    if hashed_link not in historial_oped:
        ...
        if response.status_code == 200:
            # Parse the HTML content
            soup = BeautifulSoup(response.text, 'html.parser')
            page_data = OpedDH()._process_page(soup, GEMINI_API_KEY, output_path='DATA', output_md=True)
            message, pdf_path = OpedDH()._message(page_data, 
